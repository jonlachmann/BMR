  ...
  if (plot) {
    ...
    if (back_data > 0) {
      ...
      for (i in 1:M) {
        ...
        #
        print(ggplot(data = FCDF, aes(x = Time)) +
          xlab("Time") +
          ylab(paste("Forecast of ", FCastName)) +
          geom_ribbon(aes(ymin = FCL, ymax = FCU), color = "blue", lty = 1, fill = "blue", alpha = 0.2, size = 0.1) +
          geom_hline(yintercept = 0, colour = "grey30") +
          geom_vline(xintercept = as.numeric(nrow(Y)), linetype = "longdash") +
          geom_line(aes(y = FCM), color = "red", size = 2) +
          theme(panel.background = element_rect(fill = "white", colour = "grey15")) +
          theme(panel.grid.major = element_line(colour = "grey89")), vp = vplayout(i, 1))
        #
        Sys.sleep(0.6)
      }
    } else {
